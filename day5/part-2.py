...
if __name__ == "__main__":
    with open('./input.txt') as f:
        seat_ids = sorted([get_seat_id(line) for line in f.readlines()])

        pairs = [(left, seat_ids[index + 1])
                 for index, left in enumerate(seat_ids[:-1])]

        print(find_open_spot(pairs))
